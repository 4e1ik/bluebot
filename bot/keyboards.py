...
def main_menu_kb(is_admin: bool) -> ReplyKeyboardMarkup:
    ...
    if is_admin:
        ...
        rows.append([KeyboardButton(text="Ожидают подтверждения")])
...
def catalog_kb(items: list) -> InlineKeyboardMarkup:
    ...
    for item in items:
        ...
        if item["status"] == "pending":
            status_mark = " 🔒"
        label = f"{item['name']} — {item['price']:.0f}₽{status_mark}"
        buttons.append(
...
def item_kb(item_id: int, status: str, is_own: bool) -> InlineKeyboardMarkup:
    rows = []
    if status == "available":
        rows.append(
